Para começar a **Camada de Gerenciamento e Integração**, vamos criar a fiação e o código definitivo que une o **Cérebro do Controle Remoto (ESP32-C3)** ao **Rádio (NRF24L01)** e à **Tela TFT Colorida de 1.69"**.
Neste estágio, o Controle Remoto atuará como a central receptora (Master). Ele ficará escutando o ar através do NRF24L01 e, assim que os dados do Laboratório (ou do Tanque) chegarem, ele atualizará o display gráfico IPS em tempo real.
A estrutura de pastas para esta nova seção do GitHub será:
```text
📦 meu-ecosistema-robotico
 ┗ 📂 04-Gerenciamento-Sistema
   ┃ ┗ 📂 Controle_Central_TFT_NRF24
   ┃ ┃ ┗ 📜 Controle_Central_TFT_NRF24.ino

```
### 🔌 Esquema de Ligação Duplo (TFT + NRF24 no ESP32-C3)
Como ambos os periféricos usam o barramento **SPI**, eles compartilharão os pinos de dados (MOSI, MISO, SCK). O ESP32-C3 decidirá com quem falar abrindo ou fechando os pinos de seleção de chip individuais (TFT_CS e RADIO_CSN).
| Periférico | Pino no Componente | Pino no ESP32-C3 | Tipo de Conexão |
|---|---|---|---|
| **Comum (Ambos)** | **SCL / SCK** (Clock) | **GPIO 4** | Compartilhado (SPI SCK) |
| **Comum (Ambos)** | **SDA / MOSI** (Dados) | **GPIO 6** | Compartilhado (SPI MOSI) |
| **Apenas Rádio** | **MISO** | **GPIO 5** | Exclusivo Rádio (SPI MISO) |
| **Apenas Tela** | **TFT_CS** | **GPIO 8** | Seleção da Tela |
| **Apenas Tela** | **TFT_DC** | **GPIO 2** | Comando/Dados da Tela |
| **Apenas Tela** | **TFT_RST** | **GPIO 9** | Reset da Tela |
| **Apenas Rádio** | **RADIO_CE** | **GPIO 7** | Ativação do Rádio |
| **Apenas Rádio** | **RADIO_CSN** | **GPIO 3** | Seleção do Rádio |
### 📝 Código de Gerenciamento: Controle_Central_TFT_NRF24.ino
Este código configura o rádio e a tela simultaneamente. Ele cria uma estrutura de dados (struct) idêntica à do laboratório para receber o pacote pelo ar e renderizá-lo de forma limpa na tela TFT, com direito a uma barra de status piscante para indicar recepção ativa.
```cpp
/**
...
```
### 🛰️ O próximo passo lógico:
Para ver este código funcionar na bancada, o passo seguinte é modificar ligeiramente a **Unidade de Laboratório (ESP32 DevKit v1)** para que, além de ler os sensores, ela use o rádio NRF24L01 conectado a ela para **transmitir** a struct DadosLaboratorio usando o pipe "LAB01".
